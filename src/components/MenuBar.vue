<template>
    <ul id="menu_bar">
		...
		<template v-if="isVSCExtension">
        	...
        	<li class="mode_selector" @click="openCodeViewer(false)" title="Open as Code View">{{$t('MenuBar.Switch to Code')}}</li>
		</template>
		<template v-else-if="!portrait_view">
        	<li class="mode_selector code" :class="{selected: selected_tab == 'code'}" @click="$emit('changetab', 'code')">{{$t('MenuBar.Code')}}</li>
        	<li class="mode_selector preview" :class="{selected: selected_tab == 'preview'}" @click="$emit('changetab', 'preview')">{{$t('MenuBar.Preview')}}</li>
		</template>

		<div v-if="!portrait_view" @click="openHelpPanel()" class="mode_selector highlighting_button" :class="{selected: is_help_panel_open}" :title="$t('MenuBar.Documentation')">
			<HelpCircle :size="20" />
		</div>
		<div v-if="canShare" @click="onShareParticle" class="mode_selector highlighting_button" :title="$t('MenuBar.Share')">
			<Share2 :size="20" />
		</div>
    </ul>
</template>
